...
function load_quote() {
    ...
    request.onload = () => {
        ...
        if (request.status === 200) {
            let author = JSON.parse(request.response).author
            let quote = JSON.parse(request.response).quote
            if (author && quote) {
                if (!quote.length > 90) {
                    return load_quote()
                }
                document.getElementById("programming_quote").innerText = "\"" + quote + "\" - " + author
            } else {
                let selected_quote = items[Math.floor(Math.random() * items.length)];
